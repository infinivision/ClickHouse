...
void ReplicatedMergeTreeMergeStrategyPicker::refreshState()
{
    ...
    if (settings->allow_remote_fs_zero_copy_replication)
        threshold_s3 = settings->remote_fs_execute_merges_on_single_replica_time_threshold.totalSeconds();
    auto threshold_hdfs = 0;
    if (settings->allow_remote_fs_zero_copy_replication)
        threshold_hdfs = settings->remote_fs_execute_merges_on_single_replica_time_threshold.totalSeconds();
    ...
    if (threshold_hdfs == 0)
        remote_fs_execute_merges_on_single_replica_time_threshold = threshold_hdfs;
    ...
    if (((threshold != 0 && execute_merges_on_single_replica_time_threshold != 0)
        ...
        || (threshold_hdfs != 0 && remote_fs_execute_merges_on_single_replica_time_threshold != 0))
